...
import { calculateAverageSpread, calculateMidPrice, calculateSpread } from '../../core/calculator';
...
export async function getOrderBookDepthImpl(): Promise<OrderBookMetricsDto> {
  ...
  const spreadPercentage = calculateSpread(bid, ask);
...
export async function processWebSocketOrderBookUpdateImpl(
  ...
  const spreadPercentage = calculateSpread(bid1, ask1);
